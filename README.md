...
- **高速検索**: DuckDB + Parquetの列指向ストレージで高速クエリ
- **データ圧縮**: CSV 100MB → Parquet 70.7MB（約30%削減）
...
- **DuckDB**: 組み込み型の分析データベース（SQLiteの分析版）
- **Parquet**: 列指向の圧縮データフォーマット
- **Python 3.11+**
- **依存ライブラリ**: httpx, duckdb, rich
...
国税庁から法人データ（約100MB）をダウンロードしてParquet形式に変換します。
...
**実行時間**: 約1-2分
**ダウンロード**: 法人データ5分割（全件データ）
**出力**: `~/.local/share/invoice_search_jp/invoice_data.parquet`（約70MB）
...
DuckDBの列指向ストレージとParquetフォーマットにより、必要な列だけを高速にスキャンできます。これにより、事業者名検索では約70MBのデータから約10MBのみを読み込むだけで済み、高速な検索を実現しています。

## 制限事項

- 現在は**法人データのみ**対応（個人事業主・人格なし社団等は未実装）
- 法人データの`name`カラムは空（所在地での検索が中心）
